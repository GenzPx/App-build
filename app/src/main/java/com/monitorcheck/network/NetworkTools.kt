...
object NetworkTools {
    ...
    suspend fun downloadTest(
        ...
        val url = "https://speed.cloudflare.com/__down?bytes=$sizeBytes"
        var conn: HttpURLConnection? = null
        try {
    ...
    suspend fun uploadTest(sizeBytes: Int = 2_000_000): ToolResult = withContext(Dispatchers.IO) {
        var conn: HttpURLConnection? = null
        try {
            conn = (URL("https://speed.cloudflare.com/__up").openConnection() as HttpURLConnection).apply {
